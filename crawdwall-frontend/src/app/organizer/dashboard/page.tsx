...
export default function OrganizerDashboardPage() {
  ...
  useEffect(() => {
    const fetchProposals = async () => {
      try {
        const response = await proposalAPI.getUserProposals();
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">Manage your event funding proposals</p>
      </div>
      ...
      <div className="bg-white shadow rounded-lg">
        <div className="px-4 py-5 sm:p-6">
          <h2 className="text-lg font-medium text-gray-900 mb-4">Your Recent Proposals</h2>
          {loading ? (
              ...
              {proposals.slice(0, 5).map((proposal) => (
                <li key={proposal.id} className="py-4">
                  ...
                  <div className="mt-1 text-sm text-gray-500">
                    {new Date(proposal.createdAt).toLocaleDateString()} • {proposal.amount.toLocaleString()} USD
                  </div>
                </li>
              ))}
            </ul>
